...
- If a human later changes an AI-applied decision, reconciliation records `AI Feedback Outcome=Overridden`, the observed human status and timestamp. Every processed human decision is compared with the explicit AI decision and recorded as `Matched` or `Overridden`. These records form a versioned bad-case evaluation set; production rules never self-modify online.
