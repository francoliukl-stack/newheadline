...
本节只保留产品摘要；完整、可测试定义以 [Spec §1](v3_1_event_intelligence_spec.md#1-invariants) 为准：

- INV-01 — News 是信号与首次审核入口。
- INV-02 — 当前 workspace + 钉钉 AI 表格是生产形态，不建立第二套业务库。
- INV-03 — `News=已采纳` 是发布门，战略结论另受 Evidence/Claim 门禁。
- INV-04 — 系统只能提出 P0 Candidate，最终 P0 必须人工批准。
- INV-05 — 付费调用必须通过预算预检与研究审批。
- INV-06 — Event/API Usage/Audit 不可用时失败关闭。
- INV-07 — 未通过研究门禁时只能输出 Signal Brief。
...
6. 上述优化只能减少重复读取，不得缓存或覆盖人工 `News.Status`，也不得放宽失败关闭、追溯或发布门禁。
...
9. 08:50 系统保证 News 全表每一行都有独立且明确的 `AI Status`；首次全量回填，之后仅对新增记录或输入指纹变化的记录增量更新，不覆盖人工 `Status`。09:00 卡片同时显示 AI 已采纳、已拒绝、已重复统计。
10. 人工在 11:50 前完成 `Status` 时始终以人工为准。11:50 仍为待处理时，仅允许置信度不低于 0.85、Event/URL/Publish Date/业务线/事件类型完整且 `AI Status=已采纳` 的记录自动写入最终 `Status=已采纳`；AI 已拒绝和已重复不得自动拒绝或隐藏。
11. AI 建议、最终状态、决策来源和人工覆盖结果必须留存，用于形成 Matched / Overridden bad-case 数据；规则只能通过版本化评测后离线升级，不允许在线自我修改。
...
**触发：** 周五生成研究方向与 Prompt；周六 21:00 生成 Insight 文章并发布。
...
3. 周六 21:00 只发送研究报告链接、本周关键 Event/新闻、Source URL 和 Publish Date；不再生成或发送图片 One Pager，也不重复生成长篇分析文档。
...
7. 周日任务必须接受钉钉链接字段对象，并可在精确周期标签不一致时复用最近 3 天内生成的人工 Research Queue 计划；不得回退到更早的历史报告。
8. 报告链接下方、`Weekly Key Events & News` 标题上方显示简短英文权限提示和入群链接，便于未入群读者先加入授权群后重新打开报告。

---

## 12. 质量门禁

本节不重复维护规则；质量门禁以 [Spec §1](v3_1_event_intelligence_spec.md#1-invariants) 为准：

- INV-03 — 发布必须有已采纳 News、来源日期和完整血缘。
- INV-04 — 自动化不得生成最终 P0。
- INV-05 — 付费调用与 Web Research 必须先过门禁。
- INV-06 — 关键存储缺失时不得降级绕过。
- INV-07 — 未达到研究质量门槛时标记 Signal Brief，不输出确定性战略结论。
...
| 重复发布率 | 0 |
| Publish Date 完整率 | >= 99% |
| 人工 News 审核时长 | 每日目标 <= 10 分钟；每周至少一个人工计时样本，系统估算值不得替代真实证据 |
...
| 钉钉空关键扫描冗余业务表读取 | 0 次 |
| 空 Upsert 远端读取 | 0 次 |
| 已配置 Audit Trail 的重复字段检查 | 0 次/定时任务 |
