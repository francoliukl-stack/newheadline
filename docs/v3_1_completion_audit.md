...
| 指标 | 目标 | 当前证据 | 判定 |
| --- | ---: | ---: | --- |
| 高相关外部信号 | 10–30 / 周 | 25 | 当前达标；原实时口径 40 中有 15 条只关联已归档 Event，现单列为排除项并保留 raw News 吞吐 |
| Event Case | 5–10 / 周 | 19 | 高于校准区间；其中包含已采纳真实事件，暂不为数量指标删除有效 Event |
...
| 人工 News 审核时长 | <=10 分钟/日 | KPI 已增加前一日批次、完成/待处理、AI 一致/覆盖和透明估算；`record_review_timing.py` 可将人工计时写入 RunLog/Audit；尚无真实每周样本 | 未完成，不得用估算或 dry-run 替代 |
