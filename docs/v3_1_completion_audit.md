...
| AI News 预审与兜底 | 已上线 | AI Status 明确三态，禁止待处理。2026-07-01 v1.3 全量重算 366/366 条：已采纳 40、已拒绝 272、已重复 54；第二次 dry-run 更新数为 0。352 条已比较记录的一致率为 77%，80 条覆盖中 73 条为“AI 拒绝→人工采纳”；差异主要为 Eventization Gap 36、Event Type Underclassified 33、AI 过度采纳 7。当前形成 4 条达到 `支持数≥5、一致率≥80%` 的规则，学习推翻基础规则时置信度封顶 0.84，不触发自动采纳。 |
...
| 自动化回归 | 已验证 | 138 个单测通过；v3.1 golden 指标全部通过，包括融资跨日聚合、旧 Event 归档、学习规则门槛、硬门禁和差异解释。 |
