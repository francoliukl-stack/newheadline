...
- Weekly Insight uses a manual ChatGPT Deep Research handoff: Friday generates research directions and a paste-ready prompt in Research Queue; the owner saves the completed report as a DingTalk document and fills `Research Document URL`; Sunday noon sends that link plus the weekly Event/news digest. Project Deep Research API and image One Pager jobs are disabled.
- Scheduler installation targets macOS `launchd`. Every day at 02:00 the system checks provider health, collects headlines, writes new URLs to DingTalk AI Table, backfills publish dates, and marks semantic duplicates. At 08:50 the system guarantees full-table AI Status coverage with an explicit `已采纳 / 已拒绝 / 已重复` recommendation and updates only changed input fingerprints; AI Status never remains pending. At 09:00 Daily News Review sends the human-review reminder; at 11:50 only high-confidence, fully traceable AI `已采纳` records may fall back to final `Status=已采纳` when the human Status is still pending. Human decisions always win and differences are retained as bad-case feedback. Every day at 12:00, `Daily Report` sends management the newly accepted Event digest and writes `Daily Report Sent At`; it contains no research analysis and leaves one hour for human checking before manual forwarding at 13:00. Friday 09:00 creates the next weekly OpenAI Deep Research proposal without calling a paid API. Saturday 14:00 can run Deep Research only after explicit approval; otherwise it exits without a paid call. Saturday noon Weekly Intelligence draft prepares the analysis report without marking records sent. Sunday noon `GBSS Weekly AI & Service Intelligence` sends the independent management analysis report and writes `Weekly Intelligence Sent At`.
...
- `Config` stores workflow configuration values for review and adjustment, including Daily News Review, Daily Report, Weekly Intelligence schedules, source/output sheet IDs, item limits, and report prompts. The internal `weekly_headlines` key is retained only for backward compatibility.
...
| Daily News Review | `REVIEW` | At 09:00, remind reviewers only about previous-day pending News linked to an Event Case |
| Daily Report | `PUBLISH` | Send the daily 12:00 incremental Event digest, then write back `Daily Report Sent At` |
