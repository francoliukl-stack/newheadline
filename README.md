...
- Scheduler installation targets macOS `launchd`. Every day at 02:00 the system checks provider health, collects headlines, writes new URLs to DingTalk AI Table, backfills publish dates, and marks semantic duplicates. At 08:50 the system writes a separate AI Status recommendation for previous-day News; at 09:00 Daily News Review sends the human-review reminder; at 11:50 only high-confidence, fully traceable recommendations may fall back to `Status=已采纳` when the human Status is still pending. Human decisions always win and differences are retained as bad-case feedback. Every day at 12:00, `Daily Report` sends management the newly accepted Event digest and writes `Daily Report Sent At`; it contains no research analysis and leaves one hour for human checking before manual forwarding at 13:00. Friday 09:00 creates the next weekly OpenAI Deep Research proposal without calling a paid API. Saturday 14:00 can run Deep Research only after explicit approval; otherwise it exits without a paid call. Saturday noon Weekly Intelligence draft prepares the analysis report without marking records sent. Sunday noon `GBSS Weekly AI & Service Intelligence` sends the independent management analysis report and writes `Weekly Intelligence Sent At`.
