...
- Scheduler installation targets macOS `launchd`. Daily runs check provider health, collect headlines, write new URLs to DingTalk AI Table, backfill publish dates, and mark semantic duplicates. Reminder runs send the pending-review count. Daily publish sends the newest accepted headline that has not been sent daily. A separate daily health check verifies provider availability, News table connectivity, and recent job failures. Friday 09:00 creates the next weekly OpenAI Deep Research proposal without calling a paid API. Saturday 14:00 can run Deep Research only after explicit approval; otherwise it exits without a paid call. Saturday noon weekly draft runs send a review copy of `GBSS Weekly AI & Service Intelligence` without marking records sent. Sunday noon weekly publish runs send the final report and write back `Weekly Sent At`.
...
- `Config` stores workflow configuration values for review and adjustment, including daily headline publishing, weekly insight schedules, source/output sheet IDs, item limits, and report prompts.
...
| 每日出刊 | `PUBLISH` | Publish the newest accepted headline not yet sent daily, then write back `Daily Sent At` |
| 周报草稿 | `REVIEW` | Send the Saturday noon draft `GBSS Weekly AI & Service Intelligence` for feedback without writeback |
| 出刊 | `PUBLISH` | Publish the Sunday noon final `GBSS Weekly AI & Service Intelligence`, then write back `Weekly Sent At` |
