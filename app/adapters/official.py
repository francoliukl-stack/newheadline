    ...
    def _rss(self, text: str, base_url: str, request: AdapterRequest) -> List[SourceSignal]:
        ...
        for item in root.findall(".//item") + root.findall(".//{*}entry"):
            ...
            if title and link:
                ...
                rows.append(SourceSignal(self.name, title, final_url, urlparse(final_url).netloc.lower().removeprefix("www."), date.strip(), query=request.query, metadata={"entity_id": request.entity_id, "source_grade": "T1"}))
        return rows
