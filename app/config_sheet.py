...
def default_config_items(settings: AppSettings) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        _item("reports.weekly_intelligence.final_schedule", "Weekly Intelligence", "Weekly report-link delivery schedule", _time_value(schedule.weekly_publish.hour, schedule.weekly_publish.minute, schedule.weekly_publish.weekdays), "schedule", "Saturday evening generates the Insight article, then sends its URL plus the accepted Event/news digest."),
