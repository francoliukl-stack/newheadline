...
6. `News.Status=已采纳` 作为唯一日常生效发布门控；人工审核优先，11:50 前仍无人处理时只允许高置信、可追溯的 `AI Status=已采纳` 兜底写入。`AI Status` 必须明确给出 `已采纳 / 已拒绝 / 已重复`，不得保留待处理；人工 `Status` 仍可为待处理。Evidence/Claim 审核和最终 P0 仍必须由人工负责。
...
钉钉 AI 表格的付费调用额度与 OpenAI 成本同样属于生产预算，而不是“免费基础设施”。系统必须优先减少 `列出多行记录` 和 `获取所有字段`：同一工作流复用已读取快照、空批次不读目标表、无新增关键新闻时不执行完整 Event 重算，已配置表结构不在每次定时任务中重复校验。优化不得改变 News 人工状态的实时读取要求，也不得以缓存替代钉钉业务主数据。
