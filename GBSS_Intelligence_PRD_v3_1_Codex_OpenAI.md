...
6. `News.Status=已采纳` 作为唯一日常生效发布门控；人工审核优先，11:50 前仍无人处理时只允许高置信、可追溯的 `AI Status=已采纳` 兜底写入。`AI Status` 与 `Status` 统一使用 `已采纳 / 已拒绝 / 待处理 / 已重复` 四态。Evidence/Claim 审核和最终 P0 仍必须由人工负责。
...
9. 08:50 系统保证 News 全表每一行都有独立 `AI Status`；首次全量回填，之后仅对新增记录或输入指纹变化的记录增量更新，不覆盖人工 `Status`。09:00 卡片同时显示 AI 四态统计。
10. 人工在 11:50 前完成 `Status` 时始终以人工为准。11:50 仍为待处理时，仅允许置信度不低于 0.85、Event/URL/Publish Date/业务线/事件类型完整且 `AI Status=已采纳` 的记录自动写入最终 `Status=已采纳`；AI 已拒绝、待处理和已重复不得自动拒绝或隐藏。
11. AI 建议、最终状态、决策来源和人工覆盖结果必须留存，用于形成 Matched / Overridden / Human Resolved bad-case 数据；规则只能通过版本化评测后离线升级，不允许在线自我修改。
