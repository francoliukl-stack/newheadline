...
6. `News.Status=已采纳` 作为唯一日常生效发布门控；人工审核优先，11:50 前仍无人处理时只允许高置信、可追溯的 `AI Status=建议采纳` 兜底写入。Evidence/Claim 审核和最终 P0 仍必须由人工负责。
...
9. 08:50 系统在独立的 `AI Status` 字段写入建议采纳、建议拒绝或建议复核，不覆盖人工 `Status`；09:00 卡片同时显示 AI 预审统计。
10. 人工在 11:50 前完成 `Status` 时始终以人工为准。11:50 仍为待处理时，仅允许置信度不低于 0.85、Event/URL/Publish Date/业务线/事件类型完整的建议采纳项自动写入 `Status=已采纳`；建议拒绝和建议复核不得自动拒绝。
