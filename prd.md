...
| 名称 | 对应动作 | 自动执行时机 |
| --- | --- | --- |
| **巡源** | 检查 Search Provider 是否有效；无效时发钉钉告警 | 每次搜集、催审前 |
| **搜集** | 从有效 Provider 获取候选新闻 | 周一至周六 02:00 |
| **入库** | 规范链接并将新 URL 写入钉钉 `News` 表 | 搜集后 |
| **校时** | 补齐每条新闻的 `Publish Date` | 入库后 |
| **合并** | 识别同一事件的不同表达，保留最早记录并标记重复项 | 校时后 |
| **催审** | 汇总 `待处理` 数量并发送钉钉提醒 | 周一至周六 09:00 |
| **出刊** | 将 `已采纳` 且未发送的新闻整理成周报并推送 | 周日 09:00 |
| **回执** | 发布成功后登记 `Publish Status` 与 `Sent At` | 出刊后 |
