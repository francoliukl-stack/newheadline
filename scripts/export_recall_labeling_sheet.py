...
from app.recall_sweep import stratified_sample  # noqa: E402
...
key = {
    f"RC-{index:03d}": {
        ...
        "stratum": "high" if row in sample["high"] else "low",
    }
    for index, row in enumerate(drawn, start=1)
}
...
print(f"exported {len(sheet)} cases to {args.out} (high={len(sample['high'])}, low={len(sample['low'])})")
