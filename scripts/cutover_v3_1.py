...
def automated_gate_failures() -> list[str]:
    failures = []
    for command in ([str(ROOT / ".venv" / "bin" / "python"), "-m", "unittest", "discover", "-s", "tests"], [str(ROOT / ".venv" / "bin" / "python"), str(ROOT / "scripts" / "run_v3_1_evaluation.py")]):
        ...
        if completed.returncode != 0:
            failures.append(completed.stderr.strip() or completed.stdout.strip() or "automated release gate failed")
    return failures


def readiness_failures(settings) -> list[str]:
    failures = []
    ...
    if not all(required):
        return ["v3.1 schema and lineage sheets must be configured before cutover"]
    event_settings = settings.model_copy(deep=True)
    event_settings.event_intelligence.weekly_input_mode = "event_cases"
    try:
        ready = load_weekly_input(event_settings, datetime.now(ZoneInfo("Asia/Kuala_Lumpur")), days=14, recent_count=1, include_sent=True, max_items=1, sent_fields=("Weekly Intelligence Sent At", "Weekly Sent At"))
...
def main() -> int:
    ...
    failures = [] if args.rollback else automated_gate_failures() + readiness_failures(settings)
    if args.dry_run:
        print(json.dumps({"mode": "dry-run", "status": "blocked" if failures else "ready", "target": target, "failures": failures}, indent=2))
        return 1 if failures else 0
    if failures:
        raise RuntimeError("cutover blocked: " + "; ".join(failures))
