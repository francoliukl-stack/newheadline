...
from app.event_intelligence import archive_stale_pending_events, archive_superseded_events, catalog_from_records, enrich_events_with_llm, eventize_records, persist_event_candidates, reconcile_terminal_event_statuses  # noqa: E402
...
try:
    ...
    runs.finish(run_id, "success", result_count=count, message=f"eventized={count}; merged={merged}; reconciled={reconciled}; archived={archived}; alerts={alerts}")
    audit.record(run_id=run_id, workflow="eventize", stage_code="EVENTIZE.complete", stage_name="Aggregate News into Event Cases", status="success", result_count=count, output_summary=f"Event Cases={count}; merged={merged}; reconciled={reconciled}; archived={archived}; alerts={alerts}")
    print(f"eventize success: events={count}; merged={merged}; reconciled={reconciled}; archived={archived}; alerts={alerts}")
